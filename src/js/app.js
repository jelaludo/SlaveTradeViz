    ...
    const drawVisualization = () => {
        ...
        slaveryData.forEach((trade, index) => {
            ...
            tradeGroup.append('text')
                ...
                .attr('x', -10)
                .attr('y', trackHeight / 2 + 20)
                .attr('text-anchor', 'end')
                .attr('fill', '#999')
                .text(`${(trade.lowEstimate/1000000).toFixed(1)}-${(trade.highEstimate/1000000).toFixed(1)}M`);
            ...
            Object.entries(sourceGroups).forEach(([source, group]) => {
                ...
                const intraPadding = nodePadding;
                const groupHeight =
                    segmentHeights.reduce((a, b) => a + b, 0) +
                    (group.length > 1 ? (group.length - 1) * intraPadding : 0);
                ...
                group.forEach((flow, idx) => {
                    ...
                    path.moveTo(startX + 10 + nodeWidth, sourceMidY);
                    ...
                    path.bezierCurveTo(
                        ...
                        controlPoint2X, destPos.midY,
                        endXNode, destPos.midY
                    );
                    ...
                    tradeGroup.append('path')
                        ...
                        .attr('stroke-width', Math.max(1, flowHeightProp * 0.6))
                        .attr('fill', 'none')
                        .on('mouseover', function(event) {
                    ...
                    localY += flowHeightProp + intraPadding;
                });

                const midY = cumulativeSourceY + groupHeight / 2;
                tradeGroup.append('text')
                    .attr('class', 'node-label')
                    .attr('x', startX - 10)
                    .attr('y', midY)
                ...
                cumulativeSourceY += groupHeight + nodePadding;
            });
        });
    };
    ...
    return (
        <div className="container">
            ...
            <p className="subtitle">
                An integrated timeline visualization showing sources, flows, and destinations<br/>
                of enslaved people from Ancient Rome to the present day
            </p>
